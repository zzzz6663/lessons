...
<div id="comment_{{$comment->id}}" class="comment">
    ...
    <div class="blog-post-comments">
        ...
        <div class="left">
            <div class="top">
                ...
                <span class="blog-post-comment-name"> {{$comment->name}} </span> <span class="date">
                    {{$comment->created_at}}
                </span>
                {{-- @if (\Illuminate\Support\Facades\Auth::check())--}}
                @if( $level<4) <a data-id="{{$comment->id}}" class="pull-left bl pointer answer_com text-gray"><i class="icon2-commenting-o"></i> پاسخ</a>
                    @endif
    ...
    @include('site_section.comment',['comments'=>$comment->child,'sub'=>'1'])
